...
| <a name="input_disk_encryption_key_id"></a> [disk\_encryption\_key\_id](#input\_disk\_encryption\_key\_id) | ID of the KMS key for cluster disk encryption | `string` | `null` | no |
| <a name="input_disk_size_autoscaling_mongocfg"></a> [disk\_size\_autoscaling\_mongocfg](#input\_disk\_size\_autoscaling\_mongocfg) | Disk size autoscaling settings for mongocfg hosts | <pre>object({<br/>    disk_size_limit           = number<br/>    emergency_usage_threshold = optional(number)<br/>    planned_usage_threshold   = optional(number)<br/>  })</pre> | `null` | no |
| <a name="input_disk_size_autoscaling_mongod"></a> [disk\_size\_autoscaling\_mongod](#input\_disk\_size\_autoscaling\_mongod) | Disk size autoscaling settings for mongod hosts | <pre>object({<br/>    disk_size_limit           = number<br/>    emergency_usage_threshold = optional(number)<br/>    planned_usage_threshold   = optional(number)<br/>  })</pre> | `null` | no |
| <a name="input_disk_size_autoscaling_mongoinfra"></a> [disk\_size\_autoscaling\_mongoinfra](#input\_disk\_size\_autoscaling\_mongoinfra) | Disk size autoscaling settings for mongoinfra hosts | <pre>object({<br/>    disk_size_limit           = number<br/>    emergency_usage_threshold = optional(number)<br/>    planned_usage_threshold   = optional(number)<br/>  })</pre> | `null` | no |
| <a name="input_disk_size_autoscaling_mongos"></a> [disk\_size\_autoscaling\_mongos](#input\_disk\_size\_autoscaling\_mongos) | Disk size autoscaling settings for mongos hosts | <pre>object({<br/>    disk_size_limit           = number<br/>    emergency_usage_threshold = optional(number)<br/>    planned_usage_threshold   = optional(number)<br/>  })</pre> | `null` | no |
...
| <a name="input_resources_mongoinfra"></a> [resources\_mongoinfra](#input\_resources\_mongoinfra) | Resources for mongoinfra hosts | <pre>object({<br/>    resource_preset_id = string<br/>    disk_size          = number<br/>    disk_type_id       = string<br/>  })</pre> | `null` | no |
...
| <a name="input_restore"></a> [restore](#input\_restore) | Restore settings for creating cluster from a backup | <pre>object({<br/>    backup_id = string<br/>    time      = optional(string)<br/>  })</pre> | `null` | no |
